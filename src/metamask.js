...
function metamask() {
   ...
   */
  jmsEthLibrary.connectMetamask = async (
    ...
    if (window.ethereum !== undefined) {
      ...
      jmsEthLibrary.ethProvider = new window.ethers.providers.Web3Provider(
        window.ethereum
      );
  ...
  // document.getElementById("metamaskLinkButton").addEventListener("click", jmsEthLibrary.connectMetamask);
  ...
  jmsEthLibrary.getEthContract = async (address, abi) => {
    if (typeof address !== "string" && address.length < 2) {
      // Not actually sure if there are any valid 2 character ENS names, but just in case. Seems unlikely though
